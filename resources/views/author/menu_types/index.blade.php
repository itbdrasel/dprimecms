		...
		<div class="col-md-12 mt-4">
			...
			<table class="table table-bordered">
                ...
                <tr>
					...
					<th class="sort" data-row="title" id="title" >Title</th>
                    <th>Alias</th>
                    <th class="text-center sort" data-row="place" id="place">Template/Place</th>
                    <th class="text-center sort" data-row="date" id="date">Date</th>
                    <th class="text-center" width="18%">Manage</th>
                </tr>
                </thead>
                <tbody>
                @if ($allData->count() > 0)
				...
				@foreach ($allData as $data)
                    ...
                    <tr>
						...
						<td>{{ $data->mt_title }}</td>
						<td>{{ $data->mt_alias }}</td>
                        <td class="text-center">{{$data->mt_tmpl}}</td>
                        <td class="text-center">{{date('d-m-Y', strtotime($data->mt_date))}}</td>
                        <td class="text-center">
                            <div class="btn-group">
                                <button type="button" class="btn btn-outline-info">
                                    <a href="{{url($bUrl.'/'.$data->$tableID.'/edit')}}"><i class="fa fa-edit"></i> Edit</a>
								</button>
                                <button type="button" class="btn btn-outline-info">
                                    <a id="action" data-toggle="modal" data-target="#windowmodal" href="{{url($bUrl.'/delete/'.$data->$tableID)}}"><i class="fa fa-trash"></i> Delete</a>
                                </button>
                            </div>
                        </td>
                    </tr>
